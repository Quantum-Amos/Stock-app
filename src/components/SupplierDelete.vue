<template>
    <v-dialog v-model="dialog" persistent width="auto">
        <v-card max-width="450px" class="mt-n15">
            <v-toolbar color="secondary" title="Delete Supplier"></v-toolbar>
            <v-card-text>
                <div class="text-center my-3">
                    <p class="text-error">{{ formStore.error }}</p>
                    <h4 class="text-h6 py-5">Are you sure you want delete to Supplier "{{deleteData.name}}"?</h4>
                    ...
                    <v-row justify="center" class="mb-3 mt-2">
                        ...
                        <v-col cols="12" md="6">
                            <v-btn @click="deleteSupplier" :loading="formStore.loading" block color="secondary"
...
const deleteSupplier = async () => {
    ...
    formStore.error = ""
        await deleteRequestHandler(`/suppliers/${props.deleteData.id}`, true)
        .then((res) => {
            emit('update:dialogValue', false)
            dialog.value = false
            uiStore.response = "Supplier Deleted Successfully"

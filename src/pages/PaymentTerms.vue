...
const headers = ref<any>([
    ...
    { key: "name", title: "Name" },
    { key: "num_of_days", title: "Number Of Days" },
...
const editPaymentTerm = (data: any) => {
    editData.value = data;
    editDialog.value = true;
};

const deletePaymentTerm = (data: any) => {
    ...
    <v-responsive class="mx-auto fill-height pa-5" elevation="2">
        <v-sheet elevation="1" class="h-100" rounded="lg">
            <v-toolbar elevation="0" class="bg-toolbar">
                <v-toolbar-title class="">Payment Terms</v-toolbar-title>
                <v-btn class="bg-secondary mr-5" @click="addPaymentTerm">Add Payment Term</v-btn>
            </v-toolbar>
            <v-sheet elevation="0" rounded="0" class="">
                <v-card flat>
                    ...
                    <Loader>
                        <v-data-table :headers="headers" height="100%" fixed-header :items="purchaseStore?.paymentTerms"
                            :search="search">
                            <template v-slot:item="{ item }: any">
                                <tr>
                                    <td>{{ item?.id }}</td>
                                    <td class="text-capitalize">{{ item?.name }}</td>
                                    <td class="text-capitalize">{{ item?.num_of_days }}</td>
                                    <td>
                                        <div class="d-flex">
                                            <v-btn @click="editPaymentTerm(item)" color="remBlue" variant="text"
                                                icon="mdi-pen" hide-details="auto"></v-btn>
                                            <v-btn hide-details="auto" @click="deletePaymentTerm(item)" color="red"

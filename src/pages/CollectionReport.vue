        ...
        <v-card flat>
          <template v-slot:text>
            <v-row>
              ...
              <v-col cols="12" md="1">
                <v-btn @click="getCollectionReport" :loading="formStore.loading" size="large" color="secondary"
                  class="mt-1">
                  Apply
                </v-btn>
              </v-col>
          ...
          <Loader>
            <template #default>
              <v-data-table :headers="headers" height="100%" fixed-header :items="appStore.orders" :search="search">
                <template v-slot:item.restrictions="{ item } : any">
                  <v-chip :color="getColor(item.restrictions)" dark>
                    {{ item.restrictions }}
                  </v-chip>
                </template>
              </v-data-table>

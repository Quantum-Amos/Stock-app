<template>
    <v-dialog v-model="dialog" persistent width="auto">
        <v-card max-width="450px" class="mt-n15">
            <v-toolbar color="secondary" title="Delete Purchase Order Item"></v-toolbar>
            <v-card-text>
                <div class="text-center my-3">
                    <p class="text-error">{{ formStore.error }}</p>
                    <h4 class="text-h6 py-5">Are you sure you want to Purchase Order Item with Barcode "{{deleteData.barcode?.barcode}}"?</h4>
...
const deleteTitle = async () => {
    ...
    formStore.error = ""
        ...
        .then((res) => {
            ...
            await appStore.getPurchaseOrdersById(props.deleteData?.purchase_id)
        })
    }

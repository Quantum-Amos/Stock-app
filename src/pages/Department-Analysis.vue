  ...
  <v-responsive class="px-7 h-100">
    <v-form @submit.prevent="getDepartments" v-model="form">
      <v-row class="mt-3 h-100">
        ...
        <v-col cols="12" md="3">
          <VueDatePicker label="From" v-model="startDate" placeholder="Start Date" :ui="{ input: 'dp-custom-input' }"
            class="bg-white mb-3" />
        </v-col>
        <v-col cols="12" md="3">
        ...
        <v-col>
          <v-btn :disabled="!form" :loading="formStore.loading" size="large" color="secondary" class="ml-5 mt-1"
            type="submit">
            Extract
          </v-btn>
    ...
    <div class="mt-5" v-if="analysis?.length >= 1">
      <v-card class="pa-10">
        <v-card-title class="font-weight-bold text-decoration-underline text-center text-h5">DEPARTMENT COST ANALYSIS AS
          AT
          ...
          <v-row class="mt-13">
            ...
            <v-col cols="12">
              <div v-for="group in analysis">
                <v-row style="font-size: 17px" class="font-weight-medium mx-4" v-for="collection in group.values">
          ...
          <v-row class="mt-13">
            <v-col cols="12">
              <v-row style="font-size: 17px" class="font-weight-medium">
                <v-col class="text-center text-center text-h6" style="font-weight: 600">TOTAL</v-col>

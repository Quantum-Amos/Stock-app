<template>
    <v-dialog transition="dialog-top-transition" persistent max-width="450px" scrollable v-model="dialog">
        <v-card>
            <v-toolbar color="secondary" title="Edit Category"></v-toolbar>
            <v-form v-model="form" @submit.prevent="editCategory">
              <v-card-text>
                <p
...
onMounted(()=>{
  Data.value.name = props.categoryData.name
})
...
const editCategory = async () => {
    ...
    await putRequestHandler(`categories/${props.categoryData.id}`, data, true)
        .then((res) => {

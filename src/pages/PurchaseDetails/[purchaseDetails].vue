...
const validate = async() => {
    ...
    .then((res) => {
        formStore.success = `Purchase Order Successfuly Validated`
...
const cancel = async() => {
    ...
    .then((res) => {
        formStore.success = `Purchase Order Successfuly Cancelled`
        purchaseStore.purchaseOrdersById = res
    })
    .catch((error) => {
        console.error(error)
...
const send = async() => {
    ...
    .then((res) => {
        formStore.success = `Purchase Order Successfuly Sent`
        purchaseStore.purchaseOrdersById = res
    })
    .catch((error) => {
        console.error(error)
        ...
        <v-row class="align-center">
            <v-col cols="12" md="6">
                <v-timeline direction="horizontal" side="start">
                    <v-timeline-item dot-color="grey" size="small" v-if="purchaseStore?.purchaseOrdersById?.state == 'draft' || purchaseStore?.purchaseOrdersById?.state == 'sent' || purchaseStore?.purchaseOrdersById?.state=='validate'  || purchaseStore?.purchaseOrdersById?.state=='canceled'">
                        <p class="font-weight-medium">Draft</p>
                    </v-timeline-item>
                    <v-timeline-item dot-color="secondary" size="small" v-if="purchaseStore?.purchaseOrdersById?.state == 'sent' || purchaseStore?.purchaseOrdersById?.state=='validate' || purchaseStore?.purchaseOrdersById?.state=='canceled'">
                        <p class="font-weight-medium">Sent</p>
                    </v-timeline-item>
                    <v-timeline-item dot-color="success" size="small" v-if="purchaseStore?.purchaseOrdersById?.state == 'validate'  || purchaseStore?.purchaseOrdersById?.state=='canceled'">
            ...
            <v-row class="ma-3">
                ...
                <v-col cols="12" md="6">
                    <v-combobox label="Order Type" variant="outlined" :items="purchaseStore.orderTypes"
                        item-title="name" item-value="id" v-model="order_type_id" :readonly="purchaseStore?.purchaseOrdersById?.state != 'draft'"/>
                </v-col>
            </v-row>
